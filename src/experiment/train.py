...
def train(
    ...
    MAX_TRAIN_BEHAVIORS = 200
    MAX_VAL_BEHAVIORS   = 500

    train_behavior_df = train_behavior_df.head(MAX_TRAIN_BEHAVIORS)
    val_behavior_df   = val_behavior_df.head(MAX_VAL_BEHAVIORS)

    # NEW: sanity check
    #Code generated by ChatGPT, OpenAI, November 11, 2025, https://chat.openai.com/chat

    logging.info(f"transform_fn type: {type(transform_fn)}")
    assert callable(transform_fn), f"transform_fn is not callable: {transform_fn}"
