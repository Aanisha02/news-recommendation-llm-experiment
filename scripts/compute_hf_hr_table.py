...
# -------------------------------------------------
# CONFIG – change CKPT_DIR if your best model is elsewhere
# -------------------------------------------------
...
def ratio_at_k(candidate_labels, scores, k: int) -> float:
    ...
    """
    labels = np.array(candidate_labels)
    scores = np.array(scores)

    if len(labels) == 0:
        return np.nan

    k = min(k, len(scores))     # <-- IMPORTANT
...
def compute_metrics_table():
    ...
    for idx, batch in enumerate(eval_dataloader):
        ...
        # --- Length alignment: force model outputs to match impressions ---
        n_imps = len(imps)

        # If the model produced fewer scores than impressions, skip this row (very rare)
        if len(y_score) < n_imps or len(y_true) < n_imps:
            # just skip this impression instead of trying to fix it
            continue

        # Always trim model outputs down to exactly the number of impressions
        y_score = y_score[:n_imps]
        y_true = y_true[:n_imps]
        # imps stays as-is
